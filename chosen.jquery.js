...
(function() {
  ...
  AbstractChosen = (function() {
    ...
    AbstractChosen.prototype.set_default_values = function() {
      ...
      this.search_fuzzy = this.options.search_fuzzy || false;
      ...
      this.exact_match = this.options.exact_match || false;
    ...
    AbstractChosen.prototype.winnow_results = function() {
      // Ninja
      var exact_result, eregex, escapedSearchText, option, regex, results, results_group, searchText, zregex, _i, _len, _ref;
      ...
      regex = (this.search_fuzzy ? this.get_search_regex(searchText) : this.get_search_regex(escapedSearchText));
      // Ninja
      exact_result = false;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (this.include_option_in_results(option)) {
          ...
          // Ninja
          if (option.exact_match && eregex.test(option.exact_match) || !option.exact_match && eregex.test(option.html)) {
            exact_result = true;
          }
      ...
      if (searchText.length && !results) {
        ...
        if (this.selected_option_count) {
          ...
          this.search_results.append(updateWithSelected);
        } else {
          this.update_results_content("");
        }
      ...
      if (this.create_option && searchText.length && !exact_result) {
        return this.show_create_option(searchText);
      }
    };

    AbstractChosen.prototype.highlight_search_result = function(regex, zregex, value, search) {
      var result, search_string, search_text, startpos, string, text, token, tokens, _i, _j, _len, _len1;
      if (this.search_fuzzy) {
        search_text = [];
        tokens = search.split(' ').sort(function(a, b) {
          return b.length - a.length;
        });
        search_string = value.split(' ');
        for (_i = 0, _len = search_string.length; _i < _len; _i++) {
          string = search_string[_i];
          for (_j = 0, _len1 = tokens.length; _j < _len1; _j++) {
            token = tokens[_j];
            if (string.indexOf('<span>') === -1 && string.indexOf('</span>') === -1) {
              string = string.replace(new RegExp('(' + token.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&") + ')', 'gi'), '<span>$1</span>');
            }
          }
        ...
        text = value.substr(0, startpos + search.length) + '</span>' + value.substr(startpos + search.length);
        result = text.substr(0, startpos) + '<span>' + text.substr(startpos);
      }
      return result;
    };

    AbstractChosen.prototype.get_tokenized_regex_string = function(text) {
      var regex_tokens, tokens, _i, _len;
      regex_tokens = [];
      ...
      return regex_tokens.join('');
    };

    AbstractChosen.prototype.get_tokenized_search_string = function(text) {
      return text.split(' ');
    };
    ...
    AbstractChosen.prototype.get_search_regex = function(escaped_search_string) {
      ...
      if (this.search_fuzzy) {
        return new RegExp(this.get_tokenized_regex_string(escaped_search_string), 'i');
      } else {
    ...
    AbstractChosen.default_create_option_text = "Add";
  ...
  Chosen = (function(_super) {
    ...
    Chosen.prototype.select_create_option = function (terms) {
        if (!_.findWhere(this.results_data, { value: terms })) {
            ...
            this.search_container.children().val('');
        }
    };
    // Ninja
